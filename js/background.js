...
var appConfmodel = {
  ...
  homepage: {
    ...
    sites: [
      [{ title: "smartUp Gestures", url: "https://smartup.zimoapps.com/" }],
    ],
    ...
    recentht: function (call_appslist) {
      var theFunction = function () {
        ...
        chrome.history.search(
          { text: "", maxResults: parseInt(config.apps[_appname].n_num) },
          function (items) {
            _obj.ht = items;
            sub.cons[_appname] = _obj;
            sub.insertTest(_appname);
          ...
          for (var ii = 0; ii < config.mges.mges[i].selects.length; ii++) {
            if (config.mges.mges[i].selects[ii].type == "n_scroll") {
              config.mges.mges[i].mydes.value = getDefault.i18n(
                "init_scroll_" +
                  config.mges.mges[i].selects[ii].value.substr(2),
              );
            }
          }
        }
            ...
            let replace_cdata = function (str) {
              ...
              newstr =
                str.indexOf("<![CDATA[") == 0
                  ...
                  : str;
              newstr =
                newstr.indexOf("]]>") == newstr.length - 3
                  ? newstr.replace("]]>", "")
                  : newstr;
              return newstr;
            };
            data = {
              title: replace_cdata(
                xmlData.querySelector("channel>title")
                  ? DOMPurify.sanitize(
                      xmlData.querySelector("channel>title").textContent,
                    ).toString()
                  : "noname",
              ),
              link: replace_cdata(
            ...
            for (var i = 0; i < items.length; i++) {
              ...
              for (var ii = 0; ii < _nodes.length; ii++) {
                if (_nodes[ii].tagName) {
                  _object[_nodes[ii].tagName.toLowerCase()] =
                    DOMPurify.sanitize(
        ...
        fetch("https://www.poxiao.com/")
          ...
          .then((blob) => {
            ...
            reader.onload = function (e) {
              ...
              var data = [];
              var _doms = htmlData
                .querySelectorAll(".container .content ul")[0]
                .querySelectorAll("li");
              for (var i = 0; i < _doms.length; i++) {
                ...
                for (var ii = 0; ii < _doms[i].childNodes.length; ii++) {
                  _data.push(
                    DOMPurify.sanitize(
                      _doms[i].childNodes[ii].textContent,
                    ).toString(),
                  );
                }
                _data.push(
                  "https://www.poxiao.com" +
                    _doms[i].childNodes[2].getAttribute("href"),
                );
                data.push(_data);
              }
              console.log(data);
          ...
          .then((blob) => {
            ...
            reader.onload = function (e) {
              ...
              for (var i = 0; i < domInfos.length; i++) {
                ...
                for (var ii = 0; ii < _doms.length; ii++) {
                  if (_doms[ii].childNodes.length > 1) {
                    for (
                      var iii = 0;
                      iii < _doms[ii].childNodes.length;
                      iii++
                    ) {
                      _data.push(
                        DOMPurify.sanitize(
                          _doms[ii].childNodes[iii].textContent,
                        ).toString(),
                      );
                    }
                  } else {
                    _data.push(
                      DOMPurify.sanitize(_doms[ii].textContent).toString(),
                    );
                  }
                }
              ...
              for (var i = 0; i < domDls.length - 1; i++) {
                ...
                _data.push(
                  DOMPurify.sanitize(
                    domDls[i].querySelector("td ").textContent,
                  ).toString(),
                );
                _data.push(
                  DOMPurify.sanitize(
                    domDls[i].querySelector("td input").value.substr(6),
                  ).toString(),
                );
                data.dl.push(_data);
              }
              ...
              data.name = DOMPurify.sanitize(
                htmlData.querySelector(".container #film h1").childNodes[0]
                  .textContent,
              ).toString();
        ...
        try {
          ...
          let _data = {
            ...
            imageURL:
              "https://www.bing.com" +
              DOMPurify.sanitize(
                data.querySelector("images>image>url").textContent,
              ).toString(),
    ...
    tbkjx: {
      getData: function (message, sender, sendResponse) {
        ...
        let _url = "https://quan.zimoapps.com/push/tbkjx.json",
          _configURL =
            "https://quan.zimoapps.com/push/config.json" +
            "?" +
            sub.date.getTime();
        fetch(_configURL)
          ...
          .then((json) => {
            ...
            if (
              !localStorage.getItem("tbkjx_dataversion") ||
